...
export default function ResultPanel({ result }) {
  ...
  useEffect(() => {
    if (!result) return;
    const raf = requestAnimationFrame(() => setShowAccent(true));
  ...
  return (
    <div
      className="animate-rise mt-6 rounded-xl border-l-4 bg-paper p-6 shadow-sm transition-[border-left-color] duration-300 delay-180 motion-reduce:transition-none md:p-7"
      style={{ borderLeftColor: showAccent ? cls.color : "transparent" }}
    >
